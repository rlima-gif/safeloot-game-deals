...
function Shelf({shelf,budget,sort}:{shelf:DiscoveryShelf;budget:string;sort:string}) {
  ...
  return <section className="discover-shelf" id={`selection-${shelf.id}`} aria-labelledby={`shelf-${shelf.id}`}>
...
export function DiscoveryShelves({budget,sort}:{budget:string;sort:string}) {
  ...
  return <div className="discovery-home"><div className="discover-intro"><div><span className="eyebrow">Sem ideia do que jogar?</span><h2>Seu próximo favorito está por aqui.</h2><p>Achados baratos, indies cheios de personalidade e ofertas de várias lojas. Tudo em reais.</p></div><button type="button" aria-label="Atualizar vitrines" onClick={()=>setRetry(retry+1)}><RefreshCw size={18}/></button></div>
    ...
    {error?<p role="alert">{error} <button type="button" onClick={()=>setRetry(retry+1)}>Tentar novamente</button></p>:!shelves?<div className="discovery-loading" role="status"><Gamepad2 size={28}/><span>Garimpando ofertas nas lojas…</span></div>:<><nav className="category-tiles" aria-label="Explorar coleções">{sourceShelves?.filter(s=>s.games.length && ['indie','roguelike','epic'].includes(s.id)).map(s=><a key={s.id} href={`#selection-${s.id}`}><img src={s.games[0].image} alt="" loading="lazy"/>{s.id==='indie'?'Indies':s.id==='roguelike'?'Roguelikes':'Grátis na Epic'}</a>)}</nav>{sourceShelves?.filter(s=>s.games.length).map(shelf=><Shelf key={shelf.id} shelf={shelf} budget={budget} sort={sort}/>)}{sourceShelves?.some(s=>!s.games.length) && <details className="source-details"><summary>Outras lojas e seleções sem ofertas</summary>{sourceShelves.filter(s=>!s.games.length).map(shelf=><Shelf key={shelf.id} shelf={shelf} budget={budget} sort={sort}/>)}</details>}</>}
  </div>;
}
